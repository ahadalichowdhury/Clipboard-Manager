...
class EditableTextView: NSTextView {
    ...
    override func keyDown(with event: NSEvent) {
        ...
        if event.keyCode == 36 {
            // Only insert a line break if we're in a text field
            // Otherwise, let the event propagate to be handled by handleKeyEvent
            if let editor = window?.firstResponder as? NSTextView,
               editor != self.containerView {
                // We're in a text field, insert a line break
                self.insertText("\n", replacementRange: self.selectedRange)
            } else {
                // We're not in a text field, let the event propagate
    ...
    private func handleKeyEvent(_ event: NSEvent) -> Bool {
        ...
        case 36: // Return/Enter key
            if !displayItems.isEmpty && selectedItemIndex < displayItems.count {
                ...
                // For Return/Enter key, always paste the item regardless of auto-paste setting
                copyItemToClipboard(item)

                // Immediately perform paste operation
                if item.isImage {
                    PasteManager.shared.paste(isRichText: false, isImage: true)
                } else if item.isRichText || item.hasMultipleFormats {
                    PasteManager.shared.paste(isRichText: true, isImage: false)
                } else {
                    PasteManager.shared.universalPaste()
                }
